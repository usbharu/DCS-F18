use dcs_bios_const_generator::{parse_file, write_file, write_file_enum};
use getopts::Options;
use std::env;
use std::fs::File;
use std::io::Error;
...
fn main() -> Result<(), Error> {
    ...
    options.optflag("2", "version2", "use enum");
    ...
    let output = File::create(
        matches
            .opt_get_default("o", "dcs_bios_const.rs".to_string())
            .unwrap_or("dcs_bios_const.rs".to_string()),
    )?;

    if matches.opt_present("2") {
        write_file_enum(output, &vec)
    } else {
...
fn print_help(options: Options, program: String) {
    println!(
        "DCS-BIOS Const Generator Version: {}",
        env!("CARGO_PKG_VERSION")
    );
    let brief = format!(
        "{} -i <INPUT> [-i <INPUT>]... [-o <OUTPUT>] [-h] [-v]",
        program
    );
